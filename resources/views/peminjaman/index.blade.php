...
@if(Auth::user()->ruangan_id === null)
    ...
    <form method="GET" action="{{ route('peminjaman.index') }}" class="mb-4">
        <div class="relative w-full md:max-w-sm">
            ...
            <input type="text" name="katakunci" value="{{ request('katakunci') }}" placeholder="Cari nama / NIM / barang..."
    ...
    <div class="flex flex-wrap items-center gap-2 mb-4">
        @foreach(['Dipinjam', 'Dikembalikan'] as $st)
            <a href="{{ route('peminjaman.index', array_filter(['status' => $st, 'katakunci' => request('katakunci')])) }}"
                class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors {{ request('status') === $st && !request('terlambat') ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-slate-600 border-slate-300 hover:border-blue-400 hover:text-blue-700' }}">
                {{ $st }}
            </a>
        @endforeach
        <a href="{{ route('peminjaman.index', array_filter(['terlambat' => 1, 'katakunci' => request('katakunci')])) }}"
            class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors {{ request('terlambat') ? 'bg-rose-600 text-white border-rose-600' : 'bg-white text-rose-600 border-rose-300 hover:border-rose-500' }}">
            <i data-lucide="alert-triangle" class="w-3 h-3 mr-1"></i> Terlambat
        </a>
        <a href="{{ route('peminjaman.index', array_filter(['katakunci' => request('katakunci')])) }}"
            class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors {{ !request('status') && !request('terlambat') ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-slate-600 border-slate-300 hover:border-blue-400 hover:text-blue-700' }}">
            Semua
        </a>
    </div>
            ...
            <table class="w-full text-sm text-left text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100" data-reveal-stagger>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-medium text-blue-700 min-w-[12rem]">{{ $pinjam->barang->nama_barang ?? 'Barang Dihapus' }}</td>
                        @if(Auth::user()->ruangan_id === null)
                        <td class="px-6 py-4 text-slate-500">{{ optional($pinjam->barang)->ruangan->nama_ruangan ?? '-' }}</td>
                        @endif
<td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ \Carbon\Carbon::parse($pinjam->tanggal_pinjam)->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ $pinjam->tanggal_pengembalian ? \Carbon\Carbon::parse($pinjam->tanggal_pengembalian)->format('d M Y') : '-' }}</td>
                            ...
                            @if($pinjam->status_pinjam == 'Dipinjam')
                                @if($pinjam->terlambat)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-700 border border-rose-200">
                                        <i data-lucide="alert-triangle" class="w-3 h-3 mr-1"></i> Terlambat {{ $pinjam->hari_terlambat }} hari
                                    </span>
                                @else
        ...
        <div class="divide-y divide-slate-100 md:hidden" data-reveal-stagger>
            ...
            <div class="p-5 space-y-3">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold text-slate-900 truncate">{{ $pinjam->nama_peminjam }}</div>
                        <div class="text-xs text-slate-500 mt-0.5">{{ $pinjam->nim ?? '-' }}</div>
                    </div>
                    @if($pinjam->status_pinjam == 'Dipinjam')
                        @if($pinjam->terlambat)
                            <span class="shrink-0 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 text-rose-700 border border-rose-200">Terlambat {{ $pinjam->hari_terlambat }} hari</span>
                        @else
                            <span class="shrink-0 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700 border border-orange-200">Dipinjam</span>
                        @endif
                    @else
                        <span class="shrink-0 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 border border-emerald-200">Dikembalikan</span>
                    @endif
                </div>
                <div class="space-y-1 text-xs text-slate-600">
                    <div class="flex items-center">
                        <i data-lucide="box" class="w-3.5 h-3.5 text-slate-400 mr-2 shrink-0"></i>
                        <span class="font-medium text-blue-700 truncate">{{ $pinjam->barang->nama_barang ?? 'Barang Dihapus' }}</span>
                    </div>
                    @if(Auth::user()->ruangan_id === null)
                    <div class="flex items-center">
                        <i data-lucide="door-open" class="w-3.5 h-3.5 text-slate-400 mr-2 shrink-0"></i>
                        <span>{{ optional($pinjam->barang)->ruangan->nama_ruangan ?? '-' }}</span>
                    </div>
                    @endif
                    <div class="flex items-center">
                ...
                <div class="flex items-center gap-5 border-t border-slate-100 pt-3">
                    ...
                    <form action="{{ route('peminjaman.kembalikan', $pinjam->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button type="submit" class="inline-flex items-center text-emerald-600 hover:text-emerald-800 font-medium text-sm transition-colors">
                            <i data-lucide="undo-2" class="w-4 h-4 mr-1"></i> Kembalikan
                        </button>
                    </form>
                    @endif
                    <form action="{{ route('peminjaman.destroy', $pinjam->id) }}" method="POST" onsubmit="return confirm('Yakin menghapus riwayat ini?');">
                        @csrf
                        @method('DELETE')
